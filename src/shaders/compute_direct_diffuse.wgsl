    ...
    for (var sampleIdx = 0u; sampleIdx < spp; sampleIdx = sampleIdx + 1u) {
        ...
        for (var depth = maxDepth; depth > 0u; depth = depth - 1u) {
            ...
            if (rec.t > 0.0 && !isMetallic) { 
                ...
                let cosineSample: CosineDiffuse = cosine_weighted_hemisphere(rec, rng);         
                color *= rec.material.diffuse * lightSample.color * cosineSample.pdf;  
                ray = Ray(rec.p, cosineSample.dir);   
            } else {
                if (depth == maxDepth) {
                    color *= 0.0;
                }
